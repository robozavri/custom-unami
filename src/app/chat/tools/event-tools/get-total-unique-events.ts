...
import { getTotalUniqueEvents } from '@/queries/sql/events/getTotalUniqueEvents';
...
export const getTotalUniqueEventsTool = {
  name: 'get-total-unique-events',
  description: `
- Get the count of unique event types that occurred.
- Returns the number of distinct event names and their breakdown.
- Params:
...
- Returns total unique events count with detailed breakdown.
`.trim(),
  ...
  execute: async (rawParams: unknown) => {
    ...
    const result = await getTotalUniqueEvents(websiteId, startDate, endDate, event_name);
    ...
    return {
      ...
      events_breakdown: result.events,
    };
  },
};

export type GetTotalUniqueEventsTool = typeof getTotalUniqueEventsTool;
